...
int main (int argc,char **argv){
   ...
   if (argc == 3) {
	...
	// Get the input / Se solicita al usuario la secuencia de entrada de la MT
	printf("Input: ");
	fgets(input, SIZE, stdin);

   if (input[0]=='\n'){
      // Input empty, run MT like the input was 'B' / Cadena de entrada en blanco, se ejecuta la MT como si entrase un Blanco
      input[0] = 'B';
   ...
   // Initialize machine with the configuration file and the input. / Se inicializa la maquina con el fichero de configuracion y la entrada introducida por el usuario
