...
public class Job1 {
  ...
  public static class Reduce
    ...
    protected void reduce(IntWritable key, Iterable<ActiveUser> values, Context context) throws IOException, InterruptedException {
      ...
      shard = Utilities.mapFromFile(
          new FileInputStream(
              new File(Main.shardNamePrefix+key.get()).getName()));
        ...
        for (Integer neighborIndex: neighbors) {
          ...
          if (userIds.findId(neighborIndex) %
            ...
            for (Integer item: activeUser.getNonRatedItemsArray()) {
              Integer x = userIds.findId(neighborIndex);
              System.err.println("-> Trying to find neighbor: " + x);
              if (shard == null)
                System.err.println("-> shard storage is null");
              java.util.Map<Integer, Double> m = shard.get(x);
              if (m == null)
                System.err.println("-> Profile not found in shard");
              Double rating = m.get(item);

              //Double rating = shard.get(userIds.findId(neighborIndex)).get(item);
              if (rating != null) {
                logger.log(level,"Rating for item '" + item
                    + "' is: " + rating
