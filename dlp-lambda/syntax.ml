...
(* Here, pr is Support.pr which is Format.print_string *)
let rec printtm_Term outer ctx t = match t with (* printtm_Term *)
    TmIf(fi, t1, t2, t3) ->
       obox0();
       pr "if ";
...
and printtm_AppTerm outer ctx t = match t with (* printtm_AppTerm *)
    TmApp(fi, t1, t2) ->
      obox0();
...
and printtm_PathTerm outer ctx t = match t with (* printtm_PathTerm *)
    TmProj(_, t1, l) ->
      printtm_ATerm false ctx t1; pr "."; pr l
  | t -> printtm_ATerm outer ctx t

and printtm_ATerm outer ctx t = match t with (* printtm_ATerm *)
    TmTrue(_) -> pr "true"
  | TmFalse(_) -> pr "false"
  | TmVar(fi,x,n) ->
